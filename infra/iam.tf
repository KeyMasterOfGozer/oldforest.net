...
# ── rss-feed ──────────────────────────────────────────────────────────────────
resource "aws_iam_role" "rss_feed" {
  name               = "oldforest-rss-feed"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
}
resource "aws_iam_role_policy_attachment" "rss_feed_logs" {
  role       = aws_iam_role.rss_feed.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}
resource "aws_iam_role_policy" "rss_feed" {
  name   = "dynamo-read"
  role   = aws_iam_role.rss_feed.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect   = "Allow"
